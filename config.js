...
(() => {
  ...
  function cacheServerAccount(data) { if (!data || typeof data !== "object") return; const located = locate(); if (!located?.session) return; const session = located.session; if (data.id != null) session.id = data.id; if (data.name != null) session.name = data.name; if (Object.prototype.hasOwnProperty.call(data, "displayName")) session.displayName = data.displayName || null; if (data.effectiveDisplayName != null) session.effectiveDisplayName = data.effectiveDisplayName; if (data.email != null) session.email = data.email; if (data.role != null) session.role = data.role; if (data.coinBalance != null) session.coinBalance = data.coinBalance; if (data.unreadNotifications != null) session.unreadNotifications = data.unreadNotifications; if (Array.isArray(data.library)) session.library = data.library; session.serverValidatedAt = Date.now(); located.storage.setItem(STORE_KEY, JSON.stringify(session)); window.dispatchEvent(new CustomEvent("lvg:session-hydrated", { detail: session })); }
  function cachedMeResponse() { const s = read(); if (!s?.token) return null; return new Response(JSON.stringify({ success:true, data:{ id:s.id,name:s.name,displayName:s.displayName||null,effectiveDisplayName:s.effectiveDisplayName||s.displayName||s.name,email:s.email,role:s.role||"User",coinBalance:Number(s.coinBalance||0),unreadNotifications:Number(s.unreadNotifications||0),library:Array.isArray(s.library)?s.library:[] } }), { status:200, headers:{"Content-Type":"application/json","X-LVG-Cache":"1"} }); }
  ...
  function inspectResponse(response, url, authenticated) { try { if (response.status === 401 && authenticated) queueMicrotask(() => invalidate("server-unauthorized")); else if (response.ok && authenticated && !response.headers.get("X-LVG-Cache") && (url.includes("/api/store/me") || url.includes("/api/store/profile"))) response.clone().json().then(payload => { const data = payload?.data; if (data && !Array.isArray(data)) cacheServerAccount(data); }).catch(() => {}); } catch {} }
  async function fetchMeWithTimeout(input, init, url, authenticated) { const existingSignal = init?.signal || (typeof Request !== "undefined" && input instanceof Request ? input.signal : null); const controller = existingSignal ? null : new AbortController(); const requestInit = controller ? { ...(init || {}), signal: controller.signal } : init; const timeout = controller ? setTimeout(() => controller.abort(), 8000) : null; try { const response = await nativeFetch(input, requestInit); inspectResponse(response, url, authenticated); return response.clone(); } catch (error) { const cached = cachedMeResponse(); if (cached) return cached; throw error; } finally { if (timeout) clearTimeout(timeout); } }
  window.fetch = async function(input, init) { const url = typeof input === "string" ? input : input?.url || ""; const method = String(init?.method || (typeof Request !== "undefined" && input instanceof Request ? input.method : "GET") || "GET").toUpperCase(); let headers; try { headers = new Headers(init?.headers || (typeof Request !== "undefined" && input instanceof Request ? input.headers : undefined)); } catch { headers = new Headers(); } const authenticated = url.startsWith(API_BASE) && headers.has("Authorization"); const isMeRequest = authenticated && method === "GET" && url.includes("/api/store/me"); if (isMeRequest) { if (!inflightMe) { inflightMe = fetchMeWithTimeout(input, init, url, authenticated); inflightMe.finally(() => { clearTimeout(inflightMeClearTimer); inflightMeClearTimer = setTimeout(() => { inflightMe = null; }, 1200); }).catch(() => {}); } const template = await inflightMe; return template.clone(); } const response = await nativeFetch(input, init); inspectResponse(response, url, authenticated); return response; };
  ...
  const hadSessionAtBoot = !!initial?.token; if (hadSessionAtBoot) setTimeout(() => { if (!read() && !sessionStorage.getItem(RELOAD_KEY)) refreshLoggedOutUi(); }, 2500);
})();
...
const version = "20260808-0250-modal-hidden-fix";
