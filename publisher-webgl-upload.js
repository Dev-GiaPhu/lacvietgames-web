(() => {
  ...
  function stripRoot(items){
    ...
    const index=indexes[0].path;const root=index.toLowerCase()==="index.html"?"":index.slice(0,-"index.html".length);
    const out=[];const seen=new Set();
    for(const item of valid){if(root&&!item.path.startsWith(root))continue;const p=cleanPath(root?item.path.slice(root.length):item.path);if(!p)continue;if(seen.has(p.toLowerCase()))throw new Error(`Build có file trùng đường dẫn: ${p}`);seen.add(p.toLowerCase());out.push({...item,path:p})}
    if(!out.some(x=>x.path.toLowerCase()==="index.html"))throw new Error("index.html không nằm trong thư mục build hợp lệ.");
    if(!out.some(x=>/^Build\/.+\.loader\.js$/i.test(x.path)))throw new Error("Không tìm thấy Build/*.loader.js. Hãy chọn đúng thư mục Unity WebGL Build.");
  ...
  async function zipSource(file){
    ...
    const tailSize=Math.min(file.size,66000);const tailStart=file.size-tailSize;const tail=new Uint8Array(await file.slice(tailStart).arrayBuffer());const tv=new DataView(tail.buffer,tail.byteOffset,tail.byteLength);
    let eocd=-1;for(let i=tail.length-22;i>=0;i--){if(tv.getUint32(i,true)===0x06054b50){eocd=i;break}}
    if(eocd<0)throw new Error("Không đọc được ZIP. Hãy nén build bằng ZIP chuẩn.");
    const count=tv.getUint16(eocd+10,true),cdSize=tv.getUint32(eocd+12,true),cdOffset=tv.getUint32(eocd+16,true);
    if(count===0xffff||cdSize===0xffffffff||cdOffset===0xffffffff)throw new Error("ZIP64 chưa được hỗ trợ. Hãy dùng ZIP thường hoặc Chọn thư mục build.");
    if(cdOffset+cdSize>file.size)throw new Error("Central directory của ZIP bị lỗi.");
    const cd=new Uint8Array(await file.slice(cdOffset,cdOffset+cdSize).arrayBuffer());const dv=new DataView(cd.buffer,cd.byteOffset,cd.byteLength);const dec=new TextDecoder("utf-8");let pos=0;const raw=[];
    for(let n=0;n<count;n++){
      if(pos+46>cd.length||dv.getUint32(pos,true)!==0x02014b50)throw new Error("ZIP có cấu trúc không được hỗ trợ.");
      ...
      if(flags&1)throw new Error("ZIP có mã hóa. Hãy tạo ZIP không đặt mật khẩu.");
      if(compressed===0xffffffff||size===0xffffffff||localOffset===0xffffffff)throw new Error("ZIP64 chưa được hỗ trợ.");
      if(method!==0&&method!==8)throw new Error("ZIP dùng kiểu nén không hỗ trợ. Hãy dùng ZIP Deflate chuẩn.");
      const name=cleanPath(dec.decode(cd.slice(pos+46,pos+46+nameLen)));
      if(name&&!name.endsWith("/"))raw.push({path:name,size,compressed,method,localOffset});
      pos+=46+nameLen+extraLen+commentLen;
    }
    const entries=stripRoot(raw);const byPath=new Map(entries.map(x=>[x.path,x]));
    async function blob(path){
      const e=byPath.get(path);if(!e)throw new Error(`Không đọc được ${path}`);
      const header=new DataView(await file.slice(e.localOffset,e.localOffset+30).arrayBuffer());if(header.getUint32(0,true)!==0x04034b50)throw new Error(`Local header lỗi: ${path}`);
      const dataStart=e.localOffset+30+header.getUint16(26,true)+header.getUint16(28,true);const compressedBlob=file.slice(dataStart,dataStart+e.compressed);
      if(e.method===0)return compressedBlob;
      if(typeof DecompressionStream==="undefined")throw new Error("Trình duyệt này không giải nén ZIP trực tiếp. Hãy dùng nút Chọn thư mục build.");
      try{return await new Response(compressedBlob.stream().pipeThrough(new DecompressionStream("deflate-raw"))).blob()}catch{throw new Error(`Không giải nén được ${path}. Hãy dùng ZIP Deflate chuẩn hoặc Chọn thư mục build.`)}
    }
    return{name:file.name,entries,blob,total:entries.reduce((s,x)=>s+x.size,0)};
  }

  function folderSource(fileList){
    const files=[...fileList];if(!files.length)throw new Error("Thư mục build đang trống.");
    const raw=files.map(file=>({path:cleanPath(file.webkitRelativePath||file.name),size:file.size,file})).filter(x=>x.path);const entries=stripRoot(raw);const byPath=new Map(entries.map(x=>[x.path,x.file]));
    return{name:files[0].webkitRelativePath?.split("/")[0]||"Unity WebGL folder",entries,total:entries.reduce((s,x)=>s+x.size,0),blob:async path=>{const f=byPath.get(path);if(!f)throw new Error(`Không đọc được ${path}`);return f}};
  }

  function corePaths(entries){const paths=["index.html"];const add=re=>{const x=entries.find(e=>re.test(e.path));if(x)paths.push(x.path)};add(/^Build\/.+\.loader\.js$/i);add(/^Build\/.+\.framework\.js(\.(br|gz))?$/i);add(/^Build\/.+\.wasm(\.(br|gz))?$/i);add(/^Build\/.+\.data(\.(br|gz))?$/i);return[...new Set(paths)].slice(0,8)}
  function setInfo(kind,text,err=false){const el=$(kind==="main"?"webglBuildStatus":"versionWebglStatus");if(!el)return;el.textContent=text;el.style.color=err?"#ff9dab":""}
  function setProgress(kind,value){const p=$(kind==="main"?"webglUploadProgress":"versionWebglProgress");if(!p)return;p.hidden=false;p.value=Math.max(0,Math.min(100,value||0))}
  ...
  async function upload(kind){
    ...
    try{
      ...
      for(let i=0;i<source.entries.length;i++){
        const entry=source.entries[i],ticket=ticketByPath.get(entry.path);if(!ticket)throw new Error(`Server không cấp upload URL cho ${entry.path}`);
        setInfo(kind,`Đang upload ${i+1}/${source.entries.length}: ${entry.path}`);const blob=await source.blob(entry.path);await inspectSdk(entry.path,blob,kind);await xhrPut(ticket,blob,loaded=>setProgress(kind,(done+Math.min(loaded,entry.size))*100/total));done+=entry.size;setProgress(kind,done*100/total);
      }
      button.textContent="Đang xác minh...";
      const complete=await api("/api/store/webgl-uploads/complete",{method:"POST",body:{buildRef:d.buildRef,corePaths:corePaths(source.entries)}});const playUrl=complete.data?.playUrl;if(!playUrl)throw new Error("Upload xong nhưng server chưa tạo được Play URL.");
      const target=$(kind==="main"?"playUrl":"newVersionPlayUrl");if(target){target.value=playUrl;target.dataset.managedWebgl="1"}
      setProgress(kind,100);setInfo(kind,state.sdk?"✓ Upload hoàn tất · ✓ LacVietGames SDK detected · Play URL đã tự tạo.":"✓ Upload hoàn tất · Play URL đã tự tạo. ⚠ Chưa phát hiện LacVietGames SDK trong build; nhiệm vụ thời gian chơi có thể không hoạt động.");
      const info=$(kind==="main"?"webglBuildInfo":"versionWebglInfo");if(info)info.innerHTML=`<b>${source.name}</b><span>${source.entries.length} file · ${fmtBytes(source.total)}</span><span>${state.sdk?"✓ LacVietGames SDK":"⚠ SDK chưa phát hiện"} · ✓ R2 verified</span>`;
    }catch(e){setInfo(kind,e.message||"Upload WebGL thất bại.",true)}finally{button.disabled=false;button.textContent=old}
  }

  async function chooseZip(kind,file){try{setInfo(kind,"Đang kiểm tra ZIP...");const source=await zipSource(file);states[kind].source=source;showSource(kind,source)}catch(e){states[kind].source=null;setInfo(kind,e.message,true)}}
  function chooseFolder(fileList){try{const source=folderSource(fileList);states.main.source=source;showSource("main",source)}catch(e){states.main.source=null;setInfo("main",e.message,true)}}
  ...
  function install(){
    $("webglZipFile")?.addEventListener("change",e=>{const f=e.target.files?.[0];if(f)chooseZip("main",f)});$("webglFolderFiles")?.addEventListener("change",e=>chooseFolder(e.target.files));$("uploadWebglBuild")?.addEventListener("click",()=>upload("main"));
